...
### Onze décisions architecturales fondatrices RENDEZVOUS (D1→D11)
...
| R5 | Authentification — lien magique par email | 2 sessions | ⚫ À faire — **domaine Resend dédié vérifié (D10)**, le blocage D9 est levé. Reste un préalable rapide : basculer le `from` de `notifierMartin()` vers `mail.tranquilitysuite.com` avant tout envoi à un utilisateur réel autre que Martin |
...
**Domaine d'envoi Resend de toute la suite (découvert 20/06/2026 — D9, résolu 21/06/2026 — D10)**
...
- Résolu : domaine dédié `mail.tranquilitysuite.com` acheté et vérifié (D10) — reste à activer effectivement dans le code (`from` de `notifierMartin()`)
...
**Contamination CSS inter-apps (confirmé une deuxième fois 21/06/2026)**
- Du CSS copié-collé depuis une autre app de la suite (Reviewer → RENDEZVOUS, confirmé deux fois maintenant) peut survivre indéfiniment si le sélecteur ne correspond à rien dans le HTML actuel
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer (D7) en production · domaine dédié vérifié (D10) · reste R5→R8 |
...
| A11 | **SPEC_TECHNIQUE_RENDEZVOUS_V1.md** — intégration D7→D11 | 🟡 Mineur | ⚫ À faire — la spec technique n'a toujours pas été mise à jour depuis D7 ; D8, D9, D10, D11 manquants également |
...
| B8 | **Audit transversal CSS mort RENDEZVOUS** — portée largement étendue en session du 21/06 : périmètre initial (`--color-primary` résiduel dans `.nav-btn::before`/`.nav-btn.active`, `.inscription-back-btn` orpheline) + découverte de `--info` (fantôme), de `--sidebar-bg`/`--sidebar-bg-hover`/`--sidebar-text`/`--sidebar-text-hover`/`--sidebar-border` (chacun défini jusqu'à 3× dans le fichier, dont versions en hex dur) et d'un bloc de contamination CSS copié depuis Reviewer (`.review-sidebar`, `.comments-block`, `.notes-list`, `.note-item`, `.empty-note`). Renommage `--sidebar-*` → `--topbar-*` acté (D11). Portée sur les autres apps web (Reviewer, Manifest, COVENANT, BackUpFlow) toujours non vérifiée | 🟠 Important | ✅ **Terminé — 21/06/2026** — commits `4fa40a8`, `e4646f3`, `69ded96`, `0d2e292` (repo `RENDEZVOUS`) |
| B9 | **Audit tokens CSS RENDEZVOUS vs Core** — rescopé 21/06/2026 après diff exhaustif (variables utilisées vs variables définies dans `tranquility-core`) : **24 tokens core dupliqués localement** dans `style.css` (pas seulement les 6 initialement cités : `--bg-primary`, `--bg-secondary`, `--bg-tertiary`, `--text-primary`, `--success`, `--radius-lg`), répartis sur **deux blocs `.dark-theme` distincts non fusionnés** (lignes ~60 et ~111). Le chantier inclut désormais la fusion de ces deux blocs et la purge du résidu de contamination Reviewer restant (`.comments-title`, `.empty-note`, `.note-item`, etc. — hors scope B8, volontairement non traité dans la même session). Dette antérieure à D7/Glass Drawer. Portée sur les autres apps web (Reviewer, Manifest, COVENANT, BackUpFlow) non vérifiée | 🟠 Important | 🔵 Chantier à part — non démarré, portée réévaluée le 21/06/2026 |
...
| B11 | **Domaine Resend dédié Tranquility Suite** — `tranquilitysuite.com` acheté via Cloudflare, sous-domaine `mail.tranquilitysuite.com` vérifié via Domain Connect en 9 minutes (D10). Débloque R5 — reste l'activation effective dans le code (`from` de `notifierMartin()`, toujours sur `onboarding@resend.dev`) | 🟠 Important | ✅ **Terminé — 21/06/2026** (D10) |
...
| B13 | **Activation domaine RENDEZVOUS** — basculer le `from` de `notifierMartin()` dans `submit-inscription.js` de `onboarding@resend.dev` vers une adresse `@mail.tranquilitysuite.com` | 🟠 Important | 🔵 Chantier à part — non démarré, bloquant pour un R5 pleinement fonctionnel (D10) |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer en production · domaine dédié vérifié (D10) · reste R5→R8 |

---

## Carnet de pièges — Référence

`CARNET_PIEGES_V17.md` — 21 juin 2026
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer en production (20/06) · domaine dédié vérifié (21/06) · reste R5→R8 |
...
| `CARNET_PIEGES_V17.md` | Carnet de pièges empirique | V17 — 21/06/2026 |
...
| `SPEC_TECHNIQUE_RENDEZVOUS_V1.md` | Spec technique RENDEZVOUS D1→D6 | V1.0 — 18/06/2026 ✅ ⚠️ D7/D8/D9/D10/D11 à intégrer (A11) |
...
| `SESSION_TRAVAIL_20260621_RENDEZVOUS_B11_B8.md` | Récap session — B11 livré (domaine dédié vérifié, D10), B8 livré avec portée étendue (D11, renommage `--topbar-*`), B9 rescopé (24 tokens, 2 blocs `.dark-theme`, contamination Reviewer résiduelle) | 21/06/2026 ✅ |
| `TRUE_ME.md` | Connaissance progressive Martin | Living document |

---

*Plan Directeur V5.51 · Tranquility Suite · Cellule Vidéo L'Étudiant · 21 juin 2026*
*Mise à jour — B11 livré intégralement : domaine `tranquilitysuite.com` acheté via Cloudflare, sous-domaine `mail.tranquilitysuite.com` vérifié sur Resend via Domain Connect en 9 minutes, autorisation à usage unique sans accès OAuth permanent (D10) · B8 livré avec une portée largement étendue au-delà du périmètre initial : `--info` fantôme corrigé, `--sidebar-bg`/`--sidebar-bg-hover`/`--sidebar-text`/`--sidebar-text-hover`/`--sidebar-border` (chacun dupliqué jusqu'à 3 fois, dont hex en dur) consolidés et renommés en `--topbar-*` (D11, RENDEZVOUS n'a pas de sidebar), bloc de contamination CSS copié depuis Reviewer supprimé · B9 rescopé : 24 tokens core dupliqués localement (pas 6), deux blocs `.dark-theme` à fusionner, résidu de contamination Reviewer restant à purger · B13 créé (activation effective du `from` Resend vers le nouveau domaine, bloquant pour un R5 pleinement fonctionnel) · A11 créé (mise à jour de la spec technique RENDEZVOUS avec D7→D11, jamais faite depuis D6)*

...
| R6 | État connecté — profil + flotte + statuts + heartbeat indicateur (D6) | 2 sessions | 🟢 **Examen exhaustif des combinaisons métier terminé** (D20→D31, 23-25/06/2026) — 7 kits construits (Columbia N1 ; Gumdrop, Charlie Brown, Yankee Clipper, Odyssey, Kitty Hawk, Casper en N2), 4 métiers exclus. Squelette de page déjà validé (D17-D19). Reste avant code : wording COM (remplacer "niveau N1 requis"), rate limiting B18 |
...
**Rate limiting absent sur l'authentification (découvert 22/06/2026 — R5.1)**
- `POST /auth/magic-link` n'a aucune limite de fréquence — un acteur malveillant pourrait spammer des emails vers une adresse `@letudiant.fr` active, ou saturer les quotas Resend/GitHub API
- Risque accepté consciemment pour l'usage interne actuel (zéro utilisateur réel pour l'instant) — **à traiter impérativement avant R6** (ouverture à de vrais utilisateurs)
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + frontend testés en production de bout en bout (R5.3, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19) · **examen exhaustif des combinaisons terminé** (D20→D31, 23-25/06) : 7 kits construits (Columbia, Gumdrop, Charlie Brown, Yankee Clipper, Odyssey, Kitty Hawk, Casper), 4 métiers exclus · reste rate limiting (B18) puis wording COM |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1→R5 ✅ — lien magique + frontend testés en production (R5.3, D16) · naming public harmonisé (B17) · squelette R6 validé (D17-D19) · **examen exhaustif des combinaisons terminé** (D20→D31) — 7 kits, 4 exclusions · reste rate limiting (B18) puis wording COM |
...
| B18 | **Rate limiting** sur `/auth/magic-link` et `/auth/verify` | 🔴 Critique | ⚫ À faire — impérativement avant R6 |
...
| B20 | **Nettoyage profil de test** (`pavloffmartin@gmail.com`, `profiles-private.json`) | 🟡 Mineur | ⚫ À faire — possible dès maintenant, R5.3 validé |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1→R5 ✅ (R5.3 testé en production, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19) · **examen exhaustif des combinaisons terminé** (D20→D31) — 7 kits construits, 4 métiers exclus · reste rate limiting (B18) puis wording COM |
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + intégration frontend testés en production de bout en bout (22/06, R5.3, D16) · naming public « Tranquility Suite » harmonisé (22/06, B17, D15) · squelette page connectée validé (23/06, D17-D19) · **examen exhaustif des combinaisons métier terminé** (23-25/06, D20→D31) : 7 kits construits (Columbia, Gumdrop, Charlie Brown, Yankee Clipper, Odyssey, Kitty Hawk, Casper), 4 métiers exclus · reste rate limiting (B18) puis wording COM |
...
| `CARNET_PIEGES_V23.md` | Carnet de pièges empirique | V23 — 23/06/2026 |
| `FICHE_PRODUIT_RENDEZVOUS_V1.md` | Fiche produit RENDEZVOUS | V1.5 — 25/06/2026 ✅ 7 kits détaillés (D20→D31, hors décisions transverses), tableau décisions D1→D31 fidèle au Plan Directeur, roadmap corrigée — généré, **non pushé sur GitHub** (décision Martin : ressources projet uniquement) |
...
*Plan Directeur V5.60 · Tranquility Suite · Cellule Vidéo L'Étudiant · 25 juin 2026*
*Mise à jour — extension du périmètre des combinaisons métier au-delà des 4 kits identifiés le 23/06. À la demande de Martin, balayage exhaustif de l'organigramme L'Étudiant (juin 2026) pour identifier tout métier ayant un rapport réel à la vidéo. **3 nouveaux kits construits, méthode question par question identique à celle des 4 premiers (niveau, besoin concret, apps, nom)** : ODYSSEY (Opérations spéciales, N2, D25), CASPER (Édition web et print, N2, D27, kit qui formalise un usage Covenant déjà réel sur le terrain), KITTY HAWK (Marchés hors captifs, N2, D31). **4 métiers explicitement exclus après vérification du besoin concret auprès de Martin** (pas une supposition silencieuse) : Audience et marketing automation (D26), Promotion salons (D28), Conférences et contenus (D29), Digital/Marchés captifs (D30). Total : 7 kits actés, un seul nom Apollo CSM encore disponible (ENDEAVOUR, Apollo 15). Fiche Produit RENDEZVOUS mise à jour en V1.5 en parallèle. Rewrite effectué depuis le texte intégral lu via `raw.githubusercontent.com`, conformément à la règle de méthode actée le 24/06/2026.*

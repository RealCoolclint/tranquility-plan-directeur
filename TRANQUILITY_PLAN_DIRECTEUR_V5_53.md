...
GitHub Pages (repo public RENDEZVOUS)
  ...
  Profil "active" → POST /auth/magic-link → token signé (15 min) → email avec lien
       ↓
  [À construire — R5.2] GET /auth/verify → session établie
```

**État d'implémentation (22/06/2026) :** le bloc d'écriture initial (formulaire → proxy → `profiles-private.json`) est opérationnel et testé en production, désormais avec restriction de domaine `@letudiant.fr` (B15). La notification Resend admin (`notifierAdmin()`, ex-`notifierMartin()`) envoie depuis le domaine dédié `notifications@mail.tranquilitysuite.com` (B13), testée de bout en bout. Un mécanisme temporaire de validation manuelle (`validate-profile.js`, B16) permet de faire passer un profil `pending` → `active` en attendant le vrai R7. Le module de signature de tokens (`_shared/jwt.js`) et l'endpoint `POST /auth/magic-link` sont livrés et testés en conditions réelles (anti-énumération confirmée). Repos concrets : `rendezvous-proxy` (fonction Netlify, privé) et `rendezvous-profiles-private` (données sensibles, privé), tous deux distincts des repos COVENANT. Reste à construire : `GET /auth/verify` (R5.2), intégration frontend (R5.3), et le vrai R7 (back-office, remplace B16).
...
| R5 | Authentification — lien magique par email | 2 sessions | 🟢 **R5.1 livré — 22/06/2026** : module de signature `_shared/jwt.js` (HMAC SHA-256, comparaison à temps constant) + `POST /auth/magic-link` (anti-énumération, token 15 min), testés en conditions réelles — commits `1f9368a`, `7e62fd0`. Débloqué par un mini-R7 temporaire (`validate-profile.js`, B16). Reste **R5.2** (`GET /auth/verify`) et **R5.3** (intégration frontend). Rate limiting non traité — accepté consciemment, à faire avant R6 |
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer (D7) en production · domaine vérifié ET activé (D10+B13) · restriction domaine inscription (B15) · R5.1 livré (lien magique, mini-R7 B16) · reste R5.2→R8 |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ — Glass Drawer en production · domaine activé (B13) · restriction domaine (B15) · R5.1 livré (B16 + lien magique) · reste R5.2→R8 |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer en production · domaine vérifié ET activé (D10+B13) · restriction domaine (B15) · R5.1 livré (B16 + lien magique) · reste R5.2→R8 |

---

## Carnet de pièges — Référence

`CARNET_PIEGES_V19.md` — 22 juin 2026
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer en production (20/06) · domaine vérifié ET activé (21/06) · restriction domaine (22/06, B15) · R5.1 livré (22/06, B16 + lien magique) · reste R5.2→R8 |
...
| `CARNET_PIEGES_V19.md` | Carnet de pièges empirique | V19 — 22/06/2026 |
...
*Plan Directeur V5.53 · Tranquility Suite · Cellule Vidéo L'Étudiant · 22 juin 2026*
*Mise à jour — A11 clos (V1.1→V1.3) : intégration D7→D13, correction RGPD nom de famille, réconciliation schéma réel français vs illustratif anglais, correction endpoint /register · B15 livré : restriction email @letudiant.fr à l'inscription, testée dans les deux sens · B16 livré : validate-profile.js, mini-R7 temporaire débloquant les tests R5, protégé par ADMIN_SECRET · R5.1 livré : module de signature _shared/jwt.js (HMAC SHA-256, comparaison à temps constant) + POST /auth/magic-link (anti-énumération, token 15 min), testés en conditions réelles · Décision actée : convention française du code fait foi pour profiles-private.json, pas de migration vers l'anglais · Rate limiting sur /auth/magic-link identifié comme gap, accepté consciemment, à traiter avant R6 · Reste R5.2 (verify) et R5.3 (intégration frontend)*

...
| Plan Directeur | `RealCoolclint/tranquility-plan-directeur` | `TRANQUILITY_PLAN_DIRECTEUR.md` | ✅ V5.10 |
...
| 3 | Session Recap | 1 — Fondateur | ✅ Construit — SKILL.md V1.1 — 30/05/2026 |
| 4 | Audit Complétude | 1 — Fondateur | ✅ Construit — SKILL.md V1.0 — 30/05/2026 |
| 5 | **True Me** | 1 — Fondateur | ✅ Construit — SKILL.md V1.0 + TRUE_ME.md V1.0 — 30/05/2026 |
...
2. **ARK dans APPS_CATALOG** — ajouter entrée ark avec dmgUrl dans src/renderer/app.js
3. **Mac Bureau** — cloner les 7 repos actifs dans `~/Documents/GitHub/`
...
`dmgUrl` ajouté dans `APPS_CATALOG` pour BackUpFlow et Transporter. ARK absent du catalogue principal (dans liste "bientôt") — chantier séparé à planifier.

**Bug 2 — Resend signalement ✅**
Option C actée : clé Resend dans `config.json` comme fallback. Handler `get-resend-key` dans `main.js`, exposition dans `preload.js`, UI dans `index.html` (onglet Notifications), listener dans `initSettingsModal`, pré-remplissage dans `openSettings`. Testé et validé en prod — mail reçu sur `mpavloff@letudiant.fr`.

**Commit :** `d71d4a7` — `fix: dmgUrl APPS_CATALOG + Resend fallback signalement équipe`

**Leçons :**
- Claude Desktop bloqué au démarrage → `Trust evaluate failure: leaf TemporalValidity` → fix : `sudo codesign --force --deep --sign - /Applications/Claude.app`
- Cowork tourne dans un sandbox Linux isolé — ne peut pas exécuter `gh` CLI ni les outils macOS
- `index.html` du Launcher est dans `src/renderer/`, pas à la racine
- `npm install` requis sur tout nouveau clone du Launcher (dotenv absent par défaut)
- `cd` dans le repo obligatoire avant tout `git` — ne pas lancer depuis `~`

### 30 mai 2026 (soir) — Skills Session Recap + Audit Complétude livrés

**Session Recap opérationnel.**
SKILL.md V1.1 construit et livré — 3 livrables (récap .md, actions Martin, prompt de continuation), double déclenchement auto/manuel, section Commandes Terminal obligatoire si présentes, règle mise à jour docs fondateurs intégrée. Ordre obligatoire acté : Audit Complétude → Session Recap.

**Audit Complétude opérationnel.**
SKILL.md V1.0 construit et livré — double mode (point d'étape clinique en cours de session + audit de clôture exhaustif), grille d'analyse en 6 axes (périmètre, décisions, livrables, dépendances, docs fondateurs, angles morts).

**Procédure livraison skill actée :** dossier nommé + SKILL.md + present_files = bouton "Enregistrer la compétence".

**Règle démarrage session corrigée :** question = "quel Mac ?" (Maison / Bureau / Studio) — plus jamais de question SSD.

### 30 mai 2026 (soir) — Skill True Me livré

**True Me opérationnel.**
`TRUE_ME/SKILL.md` V1.0 construit et pushé sur `RealCoolclint/true-me`. Skill installé dans les ressources du projet Claude. Architecture : SKILL.md dans Claude (les règles), TRUE_ME.md dans le repo privé (les données).

**5 comportements actifs :** adaptation du ton · anticipation des objections · challenge des angles morts · alerte de contradiction (signal ⚡, Martin décide) · comportements inférés (proposition 💡, accord obligatoire).

**Règle universelle :** tout comportement a un mode auto et un mode manuel.

**Règle de mise à jour :** commande Terminal printf prête à coller à chaque capture validée. Zéro friction, on garde l'élan.

**Premier pattern capturé :** décision par signal de longueur + recadrage timing naturel.

### 30 mai 2026 (après-midi) — Git Mac Switching + True Me lancé

**Migration workflow SSD → GitHub actée.**
Sans SSD disponible, tout le travail se fait via clone GitHub local. Dossier standard `~/Documents/GitHub/` sur tous les Mac. Protocole formalisé dans `PROTOCOLE_GIT_MAC_SWITCHING.md` V1.2.

**Skill True Me lancé.**
Repo `RealCoolclint/true-me` créé (privé). `TRUE_ME.md` V1.0 pushé — première capture de 6 éléments constitutifs de Martin.

**Règle permanente actée :** tout repo contenant des données personnelles ou stratégiques = privé par défaut.

**Leçon Hazel :** les fichiers téléchargés depuis Claude sur Mac Maison ne tombent jamais dans `~/Downloads/` directement. Documenté dans `PROTOCOLE_GIT_MAC_SWITCHING.md`.

### 30 mai 2026 (matin) — Skills universels + Workflow Assist

Session stratégique Bloc A. Catalogue de 14 skills universels JARVIS construit. Principe de gouvernance acté : valeur universelle → skill Claude, valeur contextuelle → fichier projet. `TRANQUILITY_WORKFLOW_ASSIST.md` et instruction permanente produits.

### 16 mai 2026 — Arbitrages I5 + SSD

Chantier I5 (`managers.json`) : conception terminée, prêt pour Bloc B. Protocole récupération SSD documenté.

### 15 mai 2026 — Agence Tranquility Council

23 agents IA opérationnels. Skill Council installé. Trois modes : Solo, Cellule, Conseil.

### 14 avril 2026 — Bugs Reviewer + Manifest

Reviewer : 5 bugs corrigés (commit 57ec8d3). Manifest : Chrome WebProfileSelector + Safari autocomplete corrigés.
...
Les fichiers téléchargés depuis Claude ne tombent jamais dans `~/Downloads/` directement. Toujours chercher dans les sous-dossiers selon l'extension. Commande de secours : `find ~/Downloads -name "NOM_FICHIER" 2>/dev/null`

### Cursor — Mac Maison
`cursor` n'est pas dans le PATH. Utiliser `open -a Cursor chemin/fichier` à la place.

### Repos privés
Tout repo contenant des données personnelles, stratégiques ou sensibles = privé par défaut, sans se poser la question.
...
Safari ignore `autocomplete="off"` si l'attribut `name` contient un mot reconnu comme champ de contact. Solution : renommer avec un terme neutre.
...
| `SKILL.md` | Skill universel — True Me | V1.0 ✅ |

---

*Plan Directeur V5.12 — 30 mai 2026 — Tranquility Suite · Cellule Vidéo L'Étudiant*
*Remplace V5.11*
*Phase Mercury : ✅ Clôturée — 9 avril 2026*
*Phase Gemini : Ouverte — Bloc A ✅ (A1→A8) · B1 ✅ · B2 ✅ · B3→B5 à venir · I3 ✅ · I5 conception ✅ · ARK APPS_CATALOG en attente*
*Skills universels : True Me ✅ · Session Recap ✅ · Audit Complétude ✅*

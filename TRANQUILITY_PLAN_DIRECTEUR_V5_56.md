...
| 5 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + frontend testés en production de bout en bout (R5.3, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19, 23/06) · reste rate limiting (B18) puis construire les combinaisons métier |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1→R5 ✅ — lien magique + frontend testés en production (R5.3, D16) · naming public harmonisé (B17) · squelette R6 validé (D17-D19, 23/06) · reste rate limiting puis combinaisons métier |
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + intégration frontend testés en production de bout en bout (22/06, R5.3, D16) · naming public « Tranquility Suite » harmonisé (22/06, B17, D15) · squelette page connectée validé (23/06, D17-D19) · reste rate limiting (B18) puis construire les combinaisons métier avec la cellule |
...
| `FICHE_PRODUIT_RENDEZVOUS_V1.md` | Fiche produit RENDEZVOUS | V1.1 — 18/06/2026 ⚠️ à enrichir : héritage Launcher intégré au squelette (23/06) mais fiche non mise à jour avec D17-D19 et le concept de combinaison |
...
*Plan Directeur V5.56 · Tranquility Suite · Cellule Vidéo L'Étudiant · 23 juin 2026*
*Mise à jour — squelette de la page connectée RENDEZVOUS validé en deux passes (V1 → V2 après benchmark UX externe passé au Conseil) : en-tête avec profil réduit, bloc "Mes apps" dominant, bloc "Nouveautés" en pied de page · **Pivot architectural acté pendant la session :** D17 (la visibilité verrouillée remplace l'invisibilité par défaut, pour toute la flotte sans exception), D18 (le déblocage se demande par niveau, jamais app par app — donne enfin un déclencheur concret à la règle "Mobilité entre niveaux" actée le 14/06), D19 (le mot "niveau" disparaît de l'interface ; "combinaison" devient le seul terme visible par l'utilisateur — niveau N1→N4 redevient un outil de gouvernance interne) · Matrice accès app × niveau simplifiée de trois états à deux (✅ / 🔒) · **Reste à construire avant la reprise du code R6 : le contenu réel des combinaisons métier**, en session dédiée avec la cellule vidéo et les métiers périphériques de L'Étudiant — prochaine session*

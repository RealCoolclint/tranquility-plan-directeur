...
| 2 | **COVENANT** | Web | 🔵 En lancement — C1→C6 ✅ |
...
**PDF :** généré côté client avec `pdf-lib` — logo L'Étudiant centré, textes légaux majeur/mineur, format injecté dans "Vidéo « [format] »", signature embarquée, horodatage. Téléchargement automatique via blob au moment de la validation de signature. `window.print()` et `jsPDF` = exclus (incompatibles iPad/Safari).

**`app.js` chargé une seule fois** via `document.write` avec cache-bust — jamais de balise `<script src="app.js">` en doublon dans `index.html`.
...
| COVENANT | `app.js` chargé deux fois → `SyntaxError: Identifier already declared` | Vérifier qu'il n'y a pas de `<script src="app.js">` en bas du HTML quand le cache-bust est déjà géré via `document.write` |
| COVENANT | `window.print()` / `jsPDF` incompatibles iPad Safari | Utiliser `pdf-lib` + blob download — seule approche fiable sur iOS |
| Apps web | Cache GitHub Pages masque les corrections | Toujours tester en navigation privée après un push |
...
*Plan Directeur V5.24 · Tranquility Suite · Cellule Vidéo L'Étudiant · 6 juin 2026*

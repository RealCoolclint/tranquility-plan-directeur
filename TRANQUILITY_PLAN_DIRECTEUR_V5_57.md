...
**Reste à construire avant la reprise effective de R6 :** le contenu réel des "combinaisons" métier (Monteur, Rédaction, et les métiers périphériques de L'Étudiant au-delà de la cellule vidéo) — explicitement la prochaine session, à construire avec la cellule et avec le reste des métiers concernés. Le wording final remplaçant "niveau requis" reste aussi à construire avec COM (piste actée : "combinaison", direction validée, formulations à trouver).
...
| R6 | État connecté — profil + flotte + statuts + heartbeat indicateur (D6) | 2 sessions | 🟡 Squelette de page validé (23/06/2026, D17/D18/D19) — **reste à construire avant le code : le contenu réel des combinaisons métier**, avec la cellule vidéo et les métiers périphériques de L'Étudiant — prochaine session |
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + frontend testés en production de bout en bout (R5.3, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19, 23/06) · première combinaison métier actée — Cadreur/Monteur, N1 (D20, 23/06) · reste rate limiting (B18) puis construire les combinaisons périphériques |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1→R5 ✅ — lien magique + frontend testés en production (R5.3, D16) · naming public harmonisé (B17) · squelette R6 validé (D17-D19, 23/06) · combinaison Cadreur/Monteur actée (D20, 23/06) · reste rate limiting (B18) puis combinaisons périphériques |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1→R5 ✅ (R5.3 testé en production, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19, 23/06) · reste rate limiting (B18) puis construire les combinaisons métier avec la cellule |

---

## Carnet de pièges — Référence

`CARNET_PIEGES_V21.md` — 22 juin 2026
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + intégration frontend testés en production de bout en bout (22/06, R5.3, D16) · naming public « Tranquility Suite » harmonisé (22/06, B17, D15) · squelette page connectée validé (23/06, D17-D19) · combinaison Cadreur/Monteur actée (23/06, D20) · reste rate limiting (B18) puis combinaisons périphériques (Rédaction, RS, DA/Studio) |
...
| `CARNET_PIEGES_V21.md` | Carnet de pièges empirique | V21 — 22/06/2026 |
| `FICHE_PRODUIT_RENDEZVOUS_V1.md` | Fiche produit RENDEZVOUS | V1.2 — 23/06/2026 ✅ D17-D19 intégrés, concept de combinaison documenté, première combinaison Cadreur/Monteur ajoutée |
...
*Plan Directeur V5.57 · Tranquility Suite · Cellule Vidéo L'Étudiant · 23 juin 2026*
*Mise à jour — première combinaison métier construite avec la cellule vidéo : **D20 — Combinaison Cadreur/Monteur**, niveau interne N1 (OPÉRATEURS, conforme à la grille actée le 14/06), un seul profil pour toute la cellule sans distinction de statut, 7 apps (BackUpFlow, TransPorter, Manifest, Reviewer, ARK, Rover, Covenant) — déjà toutes en accès complet à N1 dans la matrice existante, aucune modification de matrice nécessaire · Construction volontairement progressive : cellule vidéo d'abord, cercles élargis ensuite (Rédaction vidéo, Réseaux sociaux, DA/Studio), décision Martin du 23/06 · Une inversion N1/N3 détectée et corrigée avant documentation (Loi 3 — reversion évitée, pas actée) · `FICHE_PRODUIT_RENDEZVOUS_V1.md` mis à jour en V1.2 · **Reste à construire : les combinaisons des métiers périphériques**, en session(s) dédiée(s), puis rate limiting (B18) avant toute reprise du code R6*

...
| R5 | Authentification — lien magique par email | 2 sessions | ✅ **Livré — 22/06/2026** : R5.1+R5.2 (module de signature `_shared/jwt.js`, `POST /auth/magic-link`, `GET /auth/verify`, token de session minimal 48h — D14, protection anti-réutilisation via `dernierLienConsommeA`) + **R5.3** (intégration frontend — ancien sélecteur de profil `WebProfileSelector` remplacé par `MagicLinkAuth`, D16 ; lecture du `?token=` au retour ; session réelle stockée et affichée dans le header ; déconnexion locale) — testé en conditions réelles de bout en bout en production (`https://realcoolclint.github.io/RENDEZVOUS/`), commits `ab6c4da`, `99ade19`, `e5be7a6` (repo `RENDEZVOUS`). **Rate limiting toujours non traité** — accepté consciemment, à faire avant R6. **R5-bis (pivot email+mdp, D39/D40)** : Chantier A livré/testé le 04/07 soir (mot de passe temporaire bcrypt + email, commit `87dd39b`) · **Chantier B livré le 05/07** (`login.js`, D41, commit `cea19b2`) · **Chantier C livré le 05/07** (`change-password.js`, D42, commit `0b91a7b`) · **Chantier D livré le 05/07** (formulaire email/mot de passe, écran de changement forcé verrouillé, sessionStorage, durée 9h alignée frontend+backend — D43, commits `ea5c41c`, `ff48081`, `7c4e32f`) — reste E |
| R6 | État connecté — profil + flotte + statuts + heartbeat indicateur (D6) | 2 sessions | 🟢 **Examen exhaustif des combinaisons métier terminé** (D20→D31, 23-25/06/2026) — 7 kits construits (Columbia N1 ; Gumdrop, Charlie Brown, Yankee Clipper, Odyssey, Kitty Hawk, Casper en N2), 4 métiers exclus. Squelette de page déjà validé (D17-D19). **Rate limiting B18 livré (D32, 25/06/2026)**. **Wording verrouillé (D33→D36, 26/06/2026)** — "affectation" remplace "combinaison". **Code livré le 04/07/2026** (commits `290728e`, `944ef81`) — vue connectée, 7 cartes, badges D33/D36/D37, logique niveau/apps depuis JWT (D38), CTA notification, logout retour vitrine. Testé en local (N1 ✅, N2 ✅), déployé GitHub Pages. Reste : test prod bout en bout (bloqué par le pivot D39 — inutile avant R5-bis), backend réel du CTA |
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + frontend testés en production de bout en bout (R5.3, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19) · **examen exhaustif des combinaisons terminé** (D20→D31, 23-25/06) : 7 kits construits (Columbia, Gumdrop, Charlie Brown, Yankee Clipper, Odyssey, Kitty Hawk, Casper), 4 métiers exclus · **wording verrouillé** (D33→D36, 26/06) · **code R6 livré + JWT enrichi** (D37/D38, 04/07) · **pivot auth email+mdp acté** (D39/D40, 04/07) — prochain chantier : R5-bis |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1→R5 ✅ — lien magique + frontend testés en production (R5.3, D16) · naming public harmonisé (B17) · squelette R6 validé (D17-D19) · **examen exhaustif des combinaisons terminé** (D20→D31) — 7 kits, 4 exclusions · **wording verrouillé** (D33→D36, 26/06) · **code R6 livré + JWT enrichi** (D37/D38, 04/07) · **pivot auth email+mdp acté** (D39/D40, 04/07) — prochain chantier : R5-bis |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1→R5 ✅ (R5.3 testé en production, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19) · **examen exhaustif des combinaisons terminé** (D20→D31) — 7 kits construits, 4 métiers exclus · **wording verrouillé** (D33→D36, 26/06) · **code R6 livré + JWT enrichi** (D37/D38, 04/07) · **pivot auth email+mdp acté** (D39/D40, 04/07) — prochain chantier : R5-bis |
...
`CARNET_PIEGES_V27_COMPLET.md` — 5 juillet 2026 · sur GitHub (repo privé `tranquility-carnet-pieges`, branche `main`) — même garantie de récupération intégrale que ce document via `gh api` (repo privé, `raw.githubusercontent.com` inaccessible sans token). Le fichier reste également disponible en ressource projet Claude, mais **le repo GitHub fait foi** en cas de divergence. V27 ajoute 3 entrées de la session R5-bis Chantiers B/C.
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + intégration frontend testés en production de bout en bout (22/06, R5.3, D16) · naming public « Tranquility Suite » harmonisé (22/06, B17, D15) · squelette page connectée validé (23/06, D17-D19) · **examen exhaustif des combinaisons métier terminé** (23-25/06, D20→D31) : 7 kits construits (Columbia, Gumdrop, Charlie Brown, Yankee Clipper, Odyssey, Kitty Hawk, Casper), 4 métiers exclus · **wording verrouillé** (D33→D36, 26/06) · **code R6 livré + JWT enrichi** (D37/D38, 04/07) · **pivot auth email+mdp acté** (D39/D40, 04/07) · **R5-bis Chantiers A (04/07), B et C (05/07) livrés** — reste D→E |
...
| `CARNET_PIEGES_V27_COMPLET.md` | Carnet de pièges empirique | V27 — 05/07/2026 ✅ sur GitHub (repo `tranquility-carnet-pieges`) — 3 nouvelles entrées (session R5-bis Chantiers B/C). **Reste à faire (ressources projet Claude, manuel) :** retirer `CARNET_PIEGES_V26_COMPLET.md`, ajouter le V27 à la place |
...
| `CARNET_PIEGES_V27_COMPLET.md` (GitHub) | Carnet de pièges — repo dédié | V27 — repo `tranquility-carnet-pieges`, branche `main` |

...
**RENDEZVOUS** est le 23e outil de la flotte. Portail web public GitHub Pages. Porte d'entrée officielle de la Tranquility Suite pour tout nouvel utilisateur.

**Deux états :**
- Non connecté → vitrine inspirante, inscription, présentation de la flotte
- Connecté → interface personnalisée, profil, apps disponibles, nouveautés

**Modèle d'inscription :** public + validation admin obligatoire. Modèle SAS — tout le monde peut frapper à la porte, Martin ouvre.

**Invitation bidirectionnelle :** demander l'accès (utilisateur) ET offrir l'accès (admin via invitation mail).

**Architecture technique :**
```
Page publique GitHub Pages
  → Formulaire (prénom, nom, email, avatar NASA)
  → Proxy Netlify (même logique que COVENANT)
  → Profil créé en statut "pending" dans profiles.json
  → Notification mail à Martin
  → Martin valide ou refuse depuis MASTER Launcher
  → Profil passe en "active" → notification utilisateur
```
...
- PlayStation Store de la suite (→ RENDEZVOUS)
...
| 5 | **RENDEZVOUS** | Web | ⚫ En projet — portail public, inscription, profil, découverte flotte |
...
| **I7** | **RENDEZVOUS** — portail public + refonte Launcher | Infrastructure | ⚫ À planifier — session dédiée requise |
...
| I7 | **RENDEZVOUS** — portail public + refonte rôle Launcher | 🔴 Critique | ⚫ À planifier — session dédiée |
...
| 23 | **RENDEZVOUS** | Web | ⚫ En projet — portail public — inscription · profil · découverte flotte · back-office admin |
...
### RENDEZVOUS — Portail public
Inscription publique + validation admin obligatoire. Invitation bidirectionnelle. Architecture proxy Netlify identique à COVENANT. Profil `pending` jusqu'à validation MASTER.
...
*Plan Directeur V5.38 · Tranquility Suite · Cellule Vidéo L'Étudiant · 14 juin 2026*
*Mise à jour — RENDEZVOUS acté (outil #23) · Launcher redéfini comme agent local silencieux · Chantier I7 ouvert*

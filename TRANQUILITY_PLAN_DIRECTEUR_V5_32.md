...
| 2 | **COVENANT** | Web | 🔵 En lancement — C1→C7 ✅ · C12 ✅ · C11 🔴 bloqué Netlify · C9/C10 ⚫ |
...
| I6 | **tranquility-web-template** | Infrastructure | 🔵 En cours — Reviewer-Assainissement ✅ 08/06/2026 · corrections UI ✅ 09/06/2026 · tranquility-web-template ⚫ |
...
- **Prochaine étape I6 :** tranquility-web-template (PRESIDENCE arbitration requise)
...
| C10 | Dettes UI Safari (autoplay, menus) | ⚫ | — |
| C11 | Coordonnées interviewé dans email récap | 🔴 Bloqué — Netlify mal lié au repo | `4f63aaa` (code prêt) |
...
| # | Chantier | Priorité | État |
|---|----------|---------|------|
| B1 | **TRANQUILITY_SERVICES_TIERS.md** — inventaire services critiques | 🔴 Critique | ✅ Terminé — 09/06/2026 — V1.2 — commit `443b846` |
...
| B7 | **Migration accès vers Bitwarden + protocole succession** | 🔴 Critique | ⚫ À faire — décision actée 09/06/2026 |
...
**I6 — Séquence :**
I6-Reviewer-Assainissement ✅ → corrections UI Reviewer ✅ → tranquility-web-template ⚫
...
**PDF :** généré côté client avec `pdf-lib` — logo L'Étudiant centré (base64 embarqué dans `logo-data.js`), textes légaux majeur/mineur, format injecté dans "Vidéo « [format] »", signature noire (lecture canal alpha), horodatage. Téléchargement via bouton manuel sur écran confirmation. `window.print()` et `jsPDF` = exclus (incompatibles iPad/Safari).
...
**Signature PDF :** lecture canal alpha du canvas SignaturePad — pixel tracé (alpha > 10) → noir opaque, pixel vide → blanc opaque. Jamais `toDataURL()` direct sur le pad blanc.

**Logo PDF :** base64 embarqué dans `assets/logo-data.js` via `window._COVENANT_LOGO_PNG_BASE64`. Fallback fetch si absent. Fond noir → blanc, texte rouge conservé.

**Remote GitHub :** `https://github.com/RealCoolclint/COVENANT.git` (majuscules).

---

## Gouvernance des accès — Décision actée 9 juin 2026

**Problème :** comptes GitHub, Netlify, Resend sur email Gmail personnel Martin — MDP dans Keychains macOS isolés (non synchronisés) — aucune copie accessible en cas de perte machine ou de départ.

**Solution actée :** Bitwarden — plan Free — coffre "Tranquility Suite" — email `mpavloff@letudiant.fr` recommandé pour la création — code de récupération imprimé au bureau.

**Cible long terme (Apollo) :** compte rôle GitHub dédié à la cellule.

**Chantier B7 ouvert.** Référence complète : `TRANQUILITY_SERVICES_TIERS.md` V1.2.
...
| COVENANT | Signature blanche invisible dans PDF — multiply inefficace | Lire canal alpha : `if (src[i+3] > 10)` → noir, sinon blanc — canvas offscreen propre |
| COVENANT | Logo PNG rouge sur fond noir invisible sur PDF blanc | Embarquer en base64 dans `logo-data.js` + convertir fond noir → blanc pixel par pixel |
| COVENANT | Remote repo renommé `covenant` → `COVENANT` | `git remote set-url origin https://github.com/RealCoolclint/COVENANT.git` |
...
| METHODE | Travailler de tête sur un document fondateur | Lire toutes les ressources sans exception avant d'écrire la première ligne |
...
| 3 | Reviewer | Web | 🟣 En orbite — assainissement CSS ✅ · corrections UI ✅ 09/06/2026 |
...
| 8 | COVENANT | Web | 🔵 En lancement — C12 ✅ · C11 🔴 bloqué Netlify · C9/C10 ⚫ |
...
| `CARNET_PIEGES_V4.md` | Carnet de pièges | ✅ Mis à jour — 09/06/2026 |
...
*Plan Directeur V5.32 · Tranquility Suite · Cellule Vidéo L'Étudiant · 9 juin 2026*
*Mise à jour — E1 close (dette fantôme) · C12 ✅ (alignement récap) · C11 🔴 bloqué Netlify (code prêt, lier repo dashboard)*

...
GitHub Pages (repo public RENDEZVOUS)
  ...
  Profil "active" → POST /auth/magic-link → token signé (15 min) → email avec lien
       ↓
  GET /auth/verify → session établie (R5.2 ✅ — 22/06/2026)
```

**État d'implémentation (22/06/2026) :** le bloc d'écriture initial (formulaire → proxy → `profiles-private.json`) est opérationnel et testé en production, désormais avec restriction de domaine `@letudiant.fr` (B15). La notification Resend admin (`notifierAdmin()`, ex-`notifierMartin()`) envoie depuis le domaine dédié `notifications@mail.tranquilitysuite.com` (B13), testée de bout en bout. Un mécanisme temporaire de validation manuelle (`validate-profile.js`, B16) permet de faire passer un profil `pending` → `active` en attendant le vrai R7. Le module de signature de tokens (`_shared/jwt.js`), l'endpoint `POST /auth/magic-link` et l'endpoint `GET /auth/verify` (vérification + émission d'un token de session, protection anti-réutilisation via le champ `dernierLienConsommeA`) sont livrés et testés en conditions réelles de bout en bout (anti-énumération confirmée, réutilisation d'un lien déjà consommé bien rejetée). Le texte visible par l'utilisateur final dans les emails (objet, expéditeur, corps) a été harmonisé sur « Tranquility Suite » — « RENDEZVOUS » reste un nom de code de production, jamais exposé (B17). Repos concrets : `rendezvous-proxy` (fonction Netlify, privé) et `rendezvous-profiles-private` (données sensibles, privé), tous deux distincts des repos COVENANT. Reste à construire : intégration frontend (R5.3), et le vrai R7 (back-office, remplace B16).

### Quinze décisions architecturales fondatrices RENDEZVOUS (D1→D15)
...
| R5 | Authentification — lien magique par email | 2 sessions | 🟢 **R5.1 + R5.2 livrés — 22/06/2026** : module de signature `_shared/jwt.js` (HMAC SHA-256, comparaison à temps constant) + `POST /auth/magic-link` (anti-énumération, token 15 min) + `GET /auth/verify` (vérification, émission d'un token de session minimal 48h — D14, protection anti-réutilisation via `dernierLienConsommeA`), testés en conditions réelles de bout en bout — commits `1f9368a`, `7e62fd0`, `ac6ba65`. Débloqué par un mini-R7 temporaire (`validate-profile.js`, B16). Reste **R5.3** (intégration frontend). Rate limiting non traité — accepté consciemment, à faire avant R6 |
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer (D7) en production · domaine vérifié ET activé (D10+B13) · restriction domaine inscription (B15) · R5.1 + R5.2 livrés (lien magique, vérification, session 48h — mini-R7 B16) · naming public harmonisé (B17, D15) · reste R5.3→R8 |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ — Glass Drawer en production · domaine activé (B13) · restriction domaine (B15) · R5.1 + R5.2 livrés (B16 + lien magique + vérification/session) · naming public harmonisé (B17) · reste R5.3→R8 |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer en production · domaine vérifié ET activé (D10+B13) · restriction domaine (B15) · R5.1 + R5.2 livrés (B16 + lien magique + vérification/session) · naming public harmonisé (B17, D15) · reste R5.3→R8 |

---

## Carnet de pièges — Référence

`CARNET_PIEGES_V20.md` — 22 juin 2026
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1 ✅ · R2 ✅ · R3 ✅ · R4 ✅ · Glass Drawer en production (20/06) · domaine vérifié ET activé (21/06) · restriction domaine (22/06, B15) · R5.1 + R5.2 livrés (22/06, lien magique + vérification + session 48h, B16) · naming public « Tranquility Suite » harmonisé (22/06, B17, D15) · reste R5.3→R8 |
...
| `CARNET_PIEGES_V20.md` | Carnet de pièges empirique | V20 — 22/06/2026 |
...
| `SPEC_TECHNIQUE_RENDEZVOUS_V1_3.md` | Spec technique RENDEZVOUS D1→D13 | V1.3 — 22/06/2026 ✅ A11 terminé — schéma réel vs illustratif réconcilié, endpoint `/register` corrigé |
...
*Plan Directeur V5.54 · Tranquility Suite · Cellule Vidéo L'Étudiant · 22 juin 2026*
*Mise à jour — R5.2 livré : `GET /auth/verify` (vérification du lien magique, émission d'un token de session minimal 48h — D14, déviation consciente de D2 documentée), protection anti-réutilisation via le champ `dernierLienConsommeA` sur le profil (écriture GitHub avant émission du token) · Naming public harmonisé (B17, D15) : « RENDEZVOUS » retiré de tous les textes lus par l'utilisateur final dans les emails (`request-magic-link.js`, `submit-inscription.js`) au profit de « Tranquility Suite » — RENDEZVOUS reste un nom de code de production ; vitrine publique vérifiée déjà conforme · Nouvelle règle permanente actée : Cursor en mode agent pour les audits transverses (un seul prompt à l'échelle d'un repo entier, jamais fichier par fichier, quand il s'agit de chercher/corriger la même chose partout) · Profil de test conservé pour R5.3 (email basculé vers une adresse personnelle réelle pour test de bout en bout) · Reste R5.3 (intégration frontend) et le rate limiting (gap connu, avant R6)*

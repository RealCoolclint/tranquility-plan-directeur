...
En parallèle de la suite, un catalogue de 14 skills universels est en construction — des réflexes de travail qui suivent Martin dans toutes ses conversations, pas seulement sur la Tranquility Suite. Principe de gouvernance : si la valeur est universelle → skill Claude. Si la valeur est contextuelle → fichier projet. Les skills sont installés dans Claude et s'activent automatiquement selon le contexte.
...
| Plan Directeur | `RealCoolclint/tranquility-plan-directeur` | `TRANQUILITY_PLAN_DIRECTEUR.md` | ✅ V5.7 |
...
| A8 | Catalogue skills universels JARVIS | ✅ Terminé — 30 mai 2026 |
...
### 30 mai 2026 — Catalogue skills universels JARVIS + automatismes workflow

**Déclencheur :** analyse de cto.new (outil IA de développement) — extraction de bonnes pratiques sans migration d'outils.

**Trois automatismes JARVIS activés (skill projet) :**
- **Mini-plan** — généré avant tout prompt Cursor multi-fichiers, 4 lignes, validé tacitement en 2 échanges
- **Carnet de pièges** — mis à jour automatiquement en fin de session, consulté silencieusement en début. Invisible pour Martin.
- **Contexte par app** — chargé au démarrage de chaque session. Bloc transverse + bloc spécifique par app.

**Fichier créé :** `TRANQUILITY_WORKFLOW_ASSIST.md` — skill projet unique, contient les trois automatismes + tous les learnings des sessions passées.

**Principe de gouvernance des skills acté (décision PRESIDENCE) :**
> Si la valeur est universelle → skill Claude. Si la valeur est contextuelle → fichier projet.
> Question systématique : *est-ce que cette connaissance n'est valable que pour ce projet, ou est-ce de la valeur ajoutée pour ce projet et d'autres ?*

**Catalogue de 14 skills universels construit et priorisé :**

| Tier | # | Skill | Statut |
|---|---|---|---|
| 1 — Fondateurs | 1 | Context Loader | ✅ Construit |
| | 2 | Session Recap | À construire |
| | 3 | Audit Complétude | À construire |
| 2 — Haute valeur | 4 | Cursor Prompt Writer | À construire |
| | 5 | Product Design | À construire |
| | 6 | Decision Framework | À construire |
| | 7 | Multi-Perspective Council | ✅ Construit |
| 3 — Utiles | 8 | Dev Session | À construire |
| | 9 | Tech Doc Writer | À construire |
| | 10 | Audit Radar | À construire |
| | 11 | R&D Benchmark | À construire |
| | 12 | GitHub Release | À construire |
| | 13 | Electron Build | À construire |
| | 14 | Doc Templates | À construire |

**Skills construits cette session :**

*Context Loader* (`context-loader/SKILL.md`) — ancre la vérité du projet à chaque début de session. Deux modes : avec projet Claude (lecture + cross-référence documentaire) / sans projet (reconstruction depuis le prompt de continuation). Produit un bloc de confirmation structuré avant tout travail. S'active sur toute conversation avec une histoire.

*Multi-Perspective Council* (`multi-perspective-council/SKILL.md`) — analyse tout problème ou décision depuis six perspectives indépendantes calibrées sur la façon de penser de Martin :
- **SYSTÈME** — le contexte global le permet-il ?
- **QUALITÉ** — qu'est-ce qui mérite d'être protégé ?
- **ARBITRAGE** — quel est le coût réel de chaque option ?
- **FRICTION** — planning, coordination, budget, image, diplomatie, résistances institutionnelles
- **CRÉATION VS BUSINESS** — ambition créative et réalité business réconciliables ?
- **STRATÉGIE** — quelle décision est la plus utile sur le long terme ?
Mode custom : perspectives définissables à la volée. Mode express : une perspective en une seconde ("ARBITRAGE ?").

**Décision sur l'Agence Tranquility :** les 23 postes restent dans le projet (trop spécifiques). Le mécanisme Council devient un skill universel distinct → Multi-Perspective Council.

### 16 mai 2026 — Première session Marketing / COM

**Première session dédiée à la communication externe de la suite.** Objectif : préparer un package complet pour NotebookLM afin de générer une vidéo motion design de présentation et un podcast audio.

**Public cible défini (décision PRESIDENCE) :** L'Étudiant 45% / Production vidéo 35% / Communauté tech-indie 20%.

**Livrables produits :**
- `BRIEF_CREATIF_NOTEBOOKLM_V3.md` — Guide narratif en 7 temps (V3 validée après 3 itérations)
- `BIBLE_COM_ADDENDUM_MAI_2026.md` — Corrections factuelles de la Bible COM (mars → mai 2026)
- `KIT_LANCEMENT_NOTEBOOKLM.md` — Checklist d'upload + instructions de génération
- `FICHE_PROFIL_ANTOINE_PALEY.md` — Fiche profil structurée

**Décisions éditoriales clés :**
- Martin discret — transition rapide du "je" au "on"
- Workflow captif AVANT l'IA dans le récit (la philosophie précède l'action)
- Identité spatiale AVANT la flotte (le décor avant les vaisseaux)
- Flotte complète : 21 outils présentés (y compris ceux en conception)
- Gamification développée : lien affectif, quête de sens, routine transformée en exploration
- Conclusion ouverte : "L'IA n'est pas un concurrent. C'est un meilleur moteur."

**Correction équipe :** Antoine Paley (adjoint/monteur senior, Cellule Vidéo) ≠ Antoine Vassas (rédacteur en chef vidéo, Rédaction). Fiche profil créée.
...
| `BRIEF_CREATIF_NOTEBOOKLM_V3.md` | Brief créatif — guide narratif pour NotebookLM | V3.0 ✅ |
| `KIT_LANCEMENT_NOTEBOOKLM.md` | Kit de lancement — checklist + instructions de génération | V1.0 ✅ |
| `TRANQUILITY_WORKFLOW_ASSIST.md` | Skill projet — trois automatismes JARVIS (mini-plan, carnet de pièges, contexte app) | V1.0 ✅ |
| `FICHE_PROFIL_ANTOINE_PALEY.md` | Fiche profil — Antoine Paley | V1.0 ✅ |

---

*Plan Directeur V5.8 — 30 mai 2026 — Tranquility Suite · Cellule Vidéo L'Étudiant*
*Remplace V5.7*
*Phase Mercury : ✅ Clôturée — 9 avril 2026*
*Phase Gemini : Ouverte — Bloc A ✅ (A1→A8) · B1 ✅ · B2 ✅ · B3→B5 à venir · I5 conception ✅ · Bugs Launcher en attente*
*COM : Première session marketing ✅ — Package NotebookLM prêt*
*Skills universels : 2/14 construits (Context Loader ✅ · Multi-Perspective Council ✅)*

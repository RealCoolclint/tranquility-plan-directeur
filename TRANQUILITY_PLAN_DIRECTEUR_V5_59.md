...
**Combinaisons métier construites — 4/4 actées (23-24/06/2026) :** COLUMBIA (Cadreur/Monteur, N1, D20) · GUMDROP (Rédaction vidéo, N2, D21) · CHARLIE BROWN (Réseaux sociaux, N2, D23) · YANKEE CLIPPER (DA/Studio, N2, D24). Voir table récapitulative sous la table des décisions. Reste avant la reprise du code R6 : le wording final remplaçant "niveau requis" (à construire avec COM, piste actée : "combinaison") et le rate limiting B18 (critique, avant toute ouverture à de vrais utilisateurs).
...
### Combinaisons métier — récapitulatif (4/4 actées)
...
| R6 | État connecté — profil + flotte + statuts + heartbeat indicateur (D6) | 2 sessions | 🟢 **4/4 combinaisons métier construites** (D20→D24, 23-24/06/2026) — Columbia (N1), Gumdrop/Charlie Brown/Yankee Clipper (N2). Squelette de page déjà validé (D17-D19). Reste avant code : wording COM (remplacer "niveau N1 requis"), rate limiting B18 |
...
| 5 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + frontend testés en production de bout en bout (R5.3, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19) · **4/4 combinaisons métier actées** (D20→D24, 23-24/06) : Columbia, Gumdrop, Charlie Brown, Yankee Clipper · reste rate limiting (B18) puis wording COM |
...
| I7 | **RENDEZVOUS** — portail public + redéfinition Launcher daemon | Infrastructure | 🟢 R1→R5 ✅ — lien magique + frontend testés en production (R5.3, D16) · naming public harmonisé (B17) · squelette R6 validé (D17-D19) · **4/4 combinaisons métier actées** (D20→D24) · reste rate limiting (B18) puis wording COM |
...
| I7 | **RENDEZVOUS** — build R1→R8 | 🔴 Critique | 🟢 R1→R5 ✅ (R5.3 testé en production, D16) · naming public harmonisé (B17, D15) · squelette R6 validé (D17-D19) · **4/4 combinaisons métier actées** (D20→D24) · reste rate limiting (B18) puis wording COM |
...
| 23 | **RENDEZVOUS** | Web | 🟢 R1→R5 ✅ — lien magique + intégration frontend testés en production de bout en bout (22/06, R5.3, D16) · naming public « Tranquility Suite » harmonisé (22/06, B17, D15) · squelette page connectée validé (23/06, D17-D19) · **4/4 combinaisons métier actées** (23-24/06, D20→D24) : Columbia, Gumdrop, Charlie Brown, Yankee Clipper · reste rate limiting (B18) puis wording COM |
...
| `FICHE_PRODUIT_RENDEZVOUS_V1.md` | Fiche produit RENDEZVOUS | V1.3 — 23/06/2026 ✅ Combinaisons détaillées (D20→D21) — généré, **non pushé sur GitHub** (décision Martin : ressources projet uniquement) |
...
*Plan Directeur V5.59 · Tranquility Suite · Cellule Vidéo L'Étudiant · 24 juin 2026*
*Mise à jour majeure — **reconstruction complète** après détection d'une perte d'information entre V5.57 (406 lignes) et V5.58 (88 lignes). Disparu sans trace dans V5.58 : décisions D1→D19, matrice d'accès complète, architecture technique RENDEZVOUS, pièges techniques récurrents, chantiers ouverts Familles A/B/I, flotte complète des 23 outils, ressources du projet. Cause identifiée : le rewrite a été reconstruit depuis la mémoire de session plutôt que depuis le texte intégral de la version précédente. **Nouvelle règle de méthode actée :** tout rewrite complet d'un document fondateur repart désormais du texte intégral, lu directement depuis le repo GitHub (`tranquility-plan-directeur` est public, accessible en lecture brute via `raw.githubusercontent.com`) — jamais de la mémoire de session ni de recherches fragmentées. Par ailleurs cette session : **D20→D24 actées** — les 4 combinaisons métier (Columbia, Gumdrop, Charlie Brown, Yankee Clipper) sont construites ; **D22** confirme que les combinaisons priment sur la matrice générique d'accès ; extension du daemon Launcher à tous les niveaux envisagée puis explicitement écartée (réaffirme D5 sans le modifier).*
